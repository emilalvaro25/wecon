...
// FIX: Add local interface to handle `isFinal` property which is not in the official type.
interface TranscriptionWithStatus {
  text: string;
  isFinal?: boolean;
}

// FIX: Use composition over inheritance for EventEmitter.
  ...
  protected onMessage(message: LiveServerMessage) {
    ...
    if (message.serverContent) {
      ...
      if (serverContent.inputTranscription) {
        // FIX: Cast to local type to safely access isFinal.
        const transcription =
          serverContent.inputTranscription as TranscriptionWithStatus;
        // FIX: Changed this.emit to this.emitter.emit
        this.emitter.emit(
          'inputTranscription',
          transcription.text,
          transcription.isFinal ?? false,
        );
        this.log(
      ...
      if (serverContent.outputTranscription) {
        // FIX: Cast to local type to safely access isFinal.
        const transcription =
          serverContent.outputTranscription as TranscriptionWithStatus;
        // FIX: Changed this.emit to this.emitter.emit
        this.emitter.emit(
          'outputTranscription',
          transcription.text,
